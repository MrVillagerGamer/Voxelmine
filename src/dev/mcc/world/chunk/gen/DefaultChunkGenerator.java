...
public class DefaultChunkGenerator implements IChunkGenerator{
	...
	public DefaultChunkGenerator(TerrainDecorationSettings decorSettings) {
	...
	public Chunk generate(int chunkX, int chunkY, int chunkZ) {
		...
		for(int x = 0; x < Chunk.SIZE; x++) {
			for(int z = 0; z < Chunk.SIZE; z++) {
				int h = (int) perlin.noise((x+chunkX*Chunk.SIZE)/256.0f, (z+chunkZ*Chunk.SIZE)/256.0f)+(int)Perlin.AMPLITUDE/2;
				...
				for(int i = -1; i <= 1; i++) {
					for(int j = -1; j <= 1; j++) {
						int h2 = (int) perlin.noise((x+chunkX*Chunk.SIZE+i)/256.0f, (z+chunkZ*Chunk.SIZE+j)/256.0f)+(int)Perlin.AMPLITUDE/2;
						if(min > h2) min = h2;
						if(max < h2) max = h2;
